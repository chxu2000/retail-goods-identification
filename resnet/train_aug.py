...
# parser.add_argument('--checkpoint_path', type=str, default='/home/twang/distribute_train/train_resnet_distribute-23_673.ckpt', help='CheckPoint file path.')
...
def create_dataset(repeat_num=1, training=True):
    ...
    if args_opt.run_distribute:
        ...
        cifar_ds_tr = ds.ImageFolderDataset(data_home, decode=True, shuffle=True, num_shards=rank_size, shard_id=rank_id)
    else:
        cifar_ds = ds.ImageFolderDataset(data_home, decode=True, shuffle=True)
        cifar_ds_tr = ds.ImageFolderDataset(data_home, decode=True, shuffle=True)

    resize_height = 224
    resize_width = 224

    resize_op = C.Resize((resize_height, resize_width)) # interpolation default BILINEAR
    RandomCrop_op = C.RandomCrop((128))
    ...
    cifar_ds = cifar_ds.map(operations=
        [resize_op, RandomCrop_op , resize_op, colorAdjust_op,gaussianBlur_op,random_vertical_op,random_horizontal_op,random_rotation_op,
        rescale_op,normalize_op,changeswap_op], 
        input_columns=["image"], output_columns=["image"])

    cifar_ds_tr = cifar_ds_tr.map(operations=
        [resize_op,random_horizontal_op,rescale_op,normalize_op,changeswap_op], 
        input_columns=["image"], output_columns=["image"])
    
    cifar_ds = cifar_ds + cifar_ds_tr
...
if __name__ == '__main__':
    ...
    if args_opt.do_train:
        ...
        ckpoint_cb = ModelCheckpoint(prefix="train_resnet-aug", directory="/home/twang/train_aug_2021-11-5", config=config_ck)
